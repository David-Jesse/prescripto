...
const RelatedDoctors = ({speciality, docId}: RelatedDoctorsProps) => {
    ...
    const [relatedDoctors, setRelatedDoctors] = useState<DoctorType[]>([]);
    ...
    return (
        <div>
            <div className={'flex flex-col items-center gap-4 text-gray-900 my-16 md:mx-10'}>
                ...
                >
                    {relatedDoctors.slice(0, 5).map((item: DoctorType) => (
                ...
                <button onClick={() => {
                    ...
                    scrollTo(0, 0)
                        ...
                        className={'bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10 cursor-pointer hover:text-gray-600/200'}>
                    more
                </button>
            </div>
        </div>
